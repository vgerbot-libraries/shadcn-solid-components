...
const getSourceCode = (path?: string) => {
  if (!path) {
    return undefined
  }

  const normalizedPath = normalizeSourcePath(path)
  return sourceFileMap[normalizedPath]
}

const getSourceLanguage = (path?: string) => {
  if (!path) {
    return 'tsx'
  }
...
const Callout = (props: ComponentProps<'div'>) => {
  return (
    <div class="border-border bg-muted/40 my-6 rounded-xl border px-4 py-3 text-sm" {...props} />
  )
}

const SourceFile = (
  props: ComponentProps<'div'> & { path: string; language?: string; class?: string },
) => {
  const [local, rest] = splitProps(props, ['path', 'language', 'class'])
  const sourceCode = getSourceCode(local.path)
  const sourceLanguage = local.language ?? getSourceLanguage(local.path)

  return (
    <div {...rest}>
      <pre
        class={`bg-muted/60 no-scrollbar overflow-x-auto rounded-2xl px-4 py-4 text-sm ${local.class ?? ''}`}
      >
        <code class={`language-${sourceLanguage}`}>
          {sourceCode ?? `// Source file not found: ${local.path}`}
        </code>
      </pre>
    </div>
  )
...
const ComponentPreview = (
  ...
  const [local, rest] = splitProps(props, ['name', 'children', 'source', 'sourceLanguage'])
  ...
  return (
    <section class="border-border my-6 overflow-hidden rounded-2xl border" {...rest}>
      ...
      <div class="bg-background px-4 py-6">
        {local.children}
        <Show when={local.source} keyed>
          {sourcePath => (
            <SourceFile path={sourcePath} language={local.sourceLanguage} class="my-6" />
          )}
        </Show>
      </div>
    </section>
...
const ComponentSource = (props: ComponentProps<'div'>) => {
  return <div class="border-border bg-muted/30 my-6 rounded-xl border p-4 text-sm" {...props} />
}
