...
Add `shadcn-solid-components` plus the Unovis packages required by your chart types:

```bash
pnpm add @unovis/solid @unovis/ts
```

Also satisfy peer dependencies (`solid-js`, Tailwind CSS v4, `tw-animate-css`). Chart colors and tooltip chrome are intended to align with your CSS variables (for example chart color tokens defined in your theme).
...
<ComponentPreview name="Chart Tooltip Demo">
